...
//mpicxx -std=c++11 -g -Wall -o filosofos_gordo filosofos_gordo.cpp
//mpirun -np  11 ./filosofos_gordo
...
void funcion_camarero(){
  int filosofos_sentados = 0, sillas = 0,
      etiq_aceptable,
      filosofo,
  ...
  while(true){
    ...
    if(filosofos_sentados < MAX_SENTADOS){                   //el maximo de filosofos comiendo es 4
      ...
      //MPI_Probe es una Comprobación bloqueante de llegada de mensaje
    }
    else{
    ...
    if(estado.MPI_TAG == etiq_sentarse){      //se le permite sentarse
      MPI_Recv(&peticion, 1, MPI_INT, filosofo, etiq_sentarse, MPI_COMM_WORLD, &estado);
      ...
      if(filosofo == id_fil_gordo){ //necesita dos sillas
         sillas++;
      }

      MPI_Ssend(&confirmacion, 1, MPI_INT, filosofo, etiq_sentarse, MPI_COMM_WORLD);

      std::cout << "\t\tFilosofo " << estado.MPI_SOURCE << " se sienta. Hay " << filosofos_sentados << " filosofos sentados (sillas: " << sillas << ")" << '\n';
    }
    else if(estado.MPI_TAG == etiq_levantarse){    //se le permite levantarse
      MPI_Recv(&peticion, 1, MPI_INT, filosofo, etiq_levantarse, MPI_COMM_WORLD, &estado);
      ...
      std::cout << "\t\tFilosofo " << estado.MPI_SOURCE << " se levanta. Hay " << filosofos_sentados << " filosofos sentados (sillas: " << sillas << ")" << '\n';

      if(mensaje){
         cout << " --> Estaban cuatro filosofos sentados y se ha levantado el filosofo " << filosofo << endl ;
      }
   }
