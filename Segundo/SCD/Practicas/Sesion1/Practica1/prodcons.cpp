...
void  funcion_hebra_productora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      ...
      sem_wait(libre);										//Debe haber espacio en el buffer intermedio
			almacen[primera_libre] = dato;			//Le damos a la primera celda libre el valor
			primera_libre++;										//Pasamos a la siguiente celda
			//Cambio respecto LIFO
				primera_libre = primera_libre % tam_vec;
			sem_signal(ocupadas);								//Indicamos que hay posiciones ocupadas para consumir
   }
}
...
void funcion_hebra_consumidora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
      int dato ;
			...
			primera_ocupada++;										//Al consumirse el dato, la celda queda libre para nuevo uso
				primera_ocupada = primera_ocupada % tam_vec;
			sem_signal(libre);									//Indicamos que queda hueco libre
